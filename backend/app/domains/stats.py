...
async def dashboard(user: dict = Depends(get_current_user)):
    executions = await db.executions.find({"organization_id": DEFAULT_ORG_ID}, {"_id": 0}).to_list(1000)
    # A content counts ONLY if a persisted, valid deliverable exists.
    valid_deliverables = await db.deliverables.count_documents(
        {"organization_id": DEFAULT_ORG_ID, "status": {"$in": ["COMPLETATO", "COMPLETATO_CON_AVVISI"]}})
    ...
    budget = await db.budgets.find_one({"id": DEFAULT_ORG_ID}) or {}
    ...
    blocked_deliverables = await db.deliverables.count_documents(
        {"organization_id": DEFAULT_ORG_ID, "status": "BLOCCATO"})
...
async def list_deliverables(user: dict = Depends(get_current_user)):
    rows = await db.deliverables.find({"organization_id": DEFAULT_ORG_ID}, {"_id": 0}).sort("created_at", -1).to_list(300)
    return rows
...
async def list_audit(limit: int = 200, user: dict = Depends(get_current_user)):
    rows = await db.audit_logs.find({"organization_id": DEFAULT_ORG_ID}, {"_id": 0}).sort("at", -1).to_list(limit)
    return rows
