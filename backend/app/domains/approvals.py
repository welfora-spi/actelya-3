...
from ..db import db
from ..deps import require_roles, get_current_user, assert_same_org
...
async def _approve_one(approval_id: str, user: dict) -> dict:
    ...
    updated = await db.approvals.find_one_and_update(
        {"id": approval_id, "organization_id": org_id, "status": "IN_ATTESA_APPROVAZIONE"},
    ...
    if not updated:
        ...
        assert_same_org(current, user, "Richiesta non trovata")
    ...
    await log_audit(org_id=org_id, user=user, action="APPROVE",
...
async def approve(approval_id: str, user: dict = Depends(require_roles(*APPROVER_ROLES))):
    ...
    assert_same_org(appr, user, "Richiesta non trovata")
    if appr.get("type") == "AZIONE_ESTERNA" and appr.get("blocked"):
...
async def reject(approval_id: str, user: dict = Depends(require_roles(*APPROVER_ROLES))):
    ...
    await log_audit(org_id=org_id, user=user, action="REJECT",
                    entity_type="approval", entity_id=approval_id)
...
async def reject_all(user: dict = Depends(require_roles(*APPROVER_ROLES))):
    org_id = user.get("organization_id") or DEFAULT_ORG_ID
    rows = await db.approvals.find(
        {"organization_id": org_id, "status": "IN_ATTESA_APPROVAZIONE"}, {"_id": 0}).to_list(300)
    count = 0
    for r in rows:
